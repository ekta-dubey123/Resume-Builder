    ...
    return (
        <>
                ...
                <div className="modal-overlay" onClick={() => setShowLogin(false)}>
                    <div className="modal-box" onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <h2 style={{ fontSize: '20px', fontWeight: 600, margin: 0 }}>Welcome back</h2>
                            <button
                                onClick={() => setShowLogin(false)}
                                style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '20px', lineHeight: 1 }}
                            >✕</button>
                        </div>
                        ...
                        <button
                            style={{
                                ...
                                gap: '10px', background: 'white', color: '#1f2937', border: 'none',
                                borderRadius: '8px', padding: '10px', fontWeight: 500, fontSize: '14px',
                        ...
                        <button
                            style={{
                                width: '100%', background: '#A6FF5D', color: '#1f2937',
                        ...
                        <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>
                            ...
                            <span style={{ color: '#A6FF5D', cursor: 'pointer' }}>Sign up free</span>
                        </p>
                    </div>
                </div>
            )}
            ...
            <header id="home" className='bg-black text-white flex flex-col items-center bg-[url("https://assets.prebuiltui.com/images/components/hero-section/hero-background-image.png")] bg-cover bg-center bg-no-repeat pb-10'>
                <nav className="flex flex-col items-center w-full">
                    <div className="flex items-center justify-between p-4 md:px-16 lg:px-24 xl:px-32 md:py-4 w-full">
                        ...
                        <a href="#home" onClick={() => scrollTo('home')}>
                            <img src="/logo.png" alt="Resume Builder Logo" className="w-12 h-10 mr-5"
                                onError={e => {
                                    e.target.style.display = 'none';
                                    e.target.nextSibling.style.display = 'block';
                                }}
                            />
                            <span style={{ display: 'none', color: 'white', fontWeight: 700, fontSize: '18px' }}>ResumeAI</span>
                        </a>
                        ...
                        <div id="menu" className={`${mobileOpen ? 'max-md:w-full' : 'max-md:w-0'} max-md:fixed max-md:top-0 max-md:z-10 max-md:left-0 max-md:transition-all max-md:duration-300 max-md:overflow-hidden max-md:h-screen max-md:bg-black/50 max-md:backdrop-blur max-md:flex-col max-md:justify-center flex items-center gap-8 text-sm`}>
                            ...
                            <a href="#home" onClick={() => scrollTo('home')} className="text-white/70 hover:text-white transition-colors duration-200">Home</a>
                            <a href="#projects" onClick={() => scrollTo('projects')} className="text-white/70 hover:text-white transition-colors duration-200">Templates</a>
                            <a href="#testimonials" onClick={() => scrollTo('testimonials')} className="text-white/70 hover:text-white transition-colors duration-200">Testimonials</a>
                            <a href="#contact" onClick={() => scrollTo('contact')} className="text-white/70 hover:text-white transition-colors duration-200 mr-6">Contact</a>
                            ...
                            {/* Get Started button */}
                            <div className='p-[0.5px] rounded-full bg-amber-200 from-white to-transparent'>
                                <button
                                    onClick={() => scrollTo('builder')}
                                    className="hidden md:flex items-center gap-2 bg-[#A6FF5D] text-gray-800 font-medium px-4 py-2.5 rounded-full text-sm transition cursor-pointer hover:bg-[#A6FF5D]/90 active:scale-95 hidden={user}"
                                >
                ...
                {/* Headline */}
                <h1 className="text-4xl md:text-[64px]/[82px] text-center max-w-4xl mt-5 leading-tight px-4 text-white">
                    Build stunning Resumes with AI in minutes
                </h1>
                <p className="text-sm md:text-base text-gray-300 text-center max-w-lg mt-4 px-4">
                    Create a professional resume with our AI-powered builder. Get personalized suggestions and design options to make your resume stand out.
                </p>
                ...
                <div className='flex gap-3 mt-8'>
                    <button
                        onClick={() => scrollTo('builder')}
                        className="bg-[#A6FF5D] hover:bg-[#A6FF5D]/90 active:scale-95 text-gray-800 font-semibold px-6 py-2.5 rounded-full text-sm transition cursor-pointer"
                    >
                        Get Started
                    </button>
                    <button
                        onClick={() => setShowLogin(true)}
                        className="bg-[#e2e7de] hover:bg-white active:scale-95 text-gray-800 font-semibold px-6 py-2.5 rounded-full text-sm transition cursor-pointer hidden={user}"
                    >
                        Login
                    </button>
                </div>
                ...
                {/* Brand logos */}
                <div className="flex flex-row items-center justify-center gap-10 md:gap-20 mx-auto mt-16 px-4 flex-wrap">
